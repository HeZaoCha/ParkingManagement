...
class CompletedParkingRecordFactory(ParkingRecordFactory):
    ...
    exit_time = factory.LazyAttribute(
        lambda obj: obj.entry_time + timezone.timedelta(hours=2)
    )
    fee = Decimal('10.00')
...
def parking_lot_with_spaces(db) -> ParkingLot:
    ...
    for i in range(10):
        ParkingSpaceFactory(
            parking_lot=lot,
            space_number=f'A{str(i + 1).zfill(3)}'
        )
    return lot
...
def multiple_parking_lots(db) -> list[ParkingLot]:
    ...
    for i in range(3):
        ...
        for j in range(5):
            ParkingSpaceFactory(
                parking_lot=lot,
                space_number=f'{chr(65 + i)}{str(j + 1).zfill(3)}'
            )
        lots.append(lot)
    return lots
