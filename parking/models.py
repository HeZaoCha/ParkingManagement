...
class ParkingSpace(models.Model):
    ...
    space_number = models.CharField(
        max_length=20, verbose_name="车位编号", help_text="车位的唯一编号"
    )
    space_type = models.CharField(
        max_length=20, choices=SPACE_TYPE_CHOICES, default="standard", verbose_name="车位类型"
    )
    is_occupied = models.BooleanField(
        default=False, verbose_name="是否占用", help_text="车位当前是否被占用"
    )
    is_reserved = models.BooleanField(
        default=False, verbose_name="是否预留", help_text="车位是否被预留"
    )
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="创建时间")
    updated_at = models.DateTimeField(auto_now=True, verbose_name="更新时间")
...
class VIPVehicle(models.Model):
    ...
    vip_type = models.CharField(
        max_length=20, choices=VIP_TYPE_CHOICES, default="employee", verbose_name="类型"
    )
    owner_name = models.CharField(
        max_length=100, verbose_name="车主姓名", help_text="车主/员工姓名"
    )
    department = models.CharField(
        max_length=100, blank=True, verbose_name="部门", help_text="所属部门（员工车辆）"
    )
    ...
    valid_from = models.DateField(verbose_name="生效日期", help_text="免费/优惠开始日期")
...
class Vehicle(models.Model):
    ...
    vehicle_type = models.CharField(
        max_length=20, choices=VEHICLE_TYPE_CHOICES, default="car", verbose_name="车辆类型"
    )
    owner_name = models.CharField(
        max_length=100, blank=True, verbose_name="车主姓名", help_text="车辆所有者的姓名"
    )
    owner_phone = models.CharField(
        max_length=20, blank=True, verbose_name="车主电话", help_text="车辆所有者的联系电话"
    )
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="创建时间")
    updated_at = models.DateTimeField(auto_now=True, verbose_name="更新时间")
    ...
    def is_vip(self) -> bool:
        """检查是否为VIP/免费停车车辆"""
        return VIPVehicle.objects.filter(license_plate=self.license_plate, is_active=True).exists()

    def get_vip_info(self) -> VIPVehicle | None:
        """获取VIP信息"""
        try:
            vip = VIPVehicle.objects.get(license_plate=self.license_plate, is_active=True)
            if vip.is_valid:
                return vip
...
class ParkingRecord(models.Model):
    ...
    vehicle = models.ForeignKey(
        Vehicle, on_delete=models.CASCADE, related_name="parking_records", verbose_name="车辆"
    )
    ...
    entry_time = models.DateTimeField(verbose_name="入场时间", help_text="车辆进入停车场的时间")
    exit_time = models.DateTimeField(
        null=True, blank=True, verbose_name="出场时间", help_text="车辆离开停车场的时间"
    )
    duration_minutes = models.PositiveIntegerField(
        null=True, blank=True, verbose_name="停车时长（分钟）", help_text="车辆停车的总时长（分钟）"
    )
    ...
    is_paid = models.BooleanField(
        default=False, verbose_name="是否已支付", help_text="停车费用是否已支付"
    )
    ...
    notes = models.TextField(
        blank=True, verbose_name="备注", help_text="关于本次停车记录的备注信息"
    )
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="创建时间")
    updated_at = models.DateTimeField(auto_now=True, verbose_name="更新时间")
    ...
    is_free_parking = models.BooleanField(
        default=False, verbose_name="免费停车", help_text="本次停车是否免费（VIP/员工车辆）"
    )
    discount_rate = models.DecimalField(
        max_digits=3,
        decimal_places=2,
    ...
    plate_province_name = models.CharField(
        max_length=50, blank=True, verbose_name="车牌省份名称", help_text="从车牌号解析的省份全称"
    )
    ...
    def calculate_fee(self) -> Decimal:
        ...
        if pricing_config:
            match pricing_config.charge_type:
                case "tiered":
                    # 阶梯制收费
                    fee = self._calculate_tiered_fee(duration_minutes, pricing_config, parking_lot)
                case "fixed" | _:
                    # 固定小时收费（原有逻辑）
    ...
    def _calculate_tiered_fee(
        self, duration_minutes: int, pricing_config: "ParkingLotPricing", parking_lot: ParkingLot
    ) -> Decimal:
        """
        计算阶梯制费用
