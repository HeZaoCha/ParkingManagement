...
class LicensePlateLocation(models.Model):
    ...
    province = models.ForeignKey(Province, on_delete=models.CASCADE, related_name='plate_locations', verbose_name='省份')
    city = models.ForeignKey(City, on_delete=models.CASCADE, related_name='plate_locations', verbose_name='地级市')
    ...
    def get_location_by_plate(cls, license_plate: str) -> dict:
        ...
        try:
            location = cls.objects.select_related('province', 'city').get(license_plate_prefix=prefix)
            return {
...
class WantedVehicle(models.Model):
    ...
    priority = models.IntegerField(default=1, verbose_name='优先级', help_text='1-10，数字越大优先级越高')
