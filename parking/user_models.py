...
class UserProfile(models.Model):
    ...
    role = models.CharField(
        max_length=20, choices=ROLE_CHOICES, default="customer", verbose_name="角色"
    )
...
class VerificationCode(models.Model):
    ...
    code_type = models.CharField(max_length=20, choices=TYPE_CHOICES, verbose_name="验证码类型")
    ...
    @classmethod
    def create_code(
        cls, code_type: str, purpose: str, target: str, expires_minutes: int = 10
    ) -> VerificationCode:
        """创建验证码"""
        # 使旧验证码失效
        cls.objects.filter(
        ...
        return cls.objects.create(
            code_type=code_type, purpose=purpose, target=target, code=code, expires_at=expires_at
        )
...
class StaffSchedule(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="schedules", verbose_name="工作人员"
    )
    ...
    def __str__(self) -> str:
        return (
            f"{self.user.username} - {self.get_weekday_display()} {self.start_time}-{self.end_time}"
        )
...
class ContactMessage(models.Model):
    ...
    message_type = models.CharField(
        max_length=20, choices=TYPE_CHOICES, default="feedback", verbose_name="消息类型"
    )
