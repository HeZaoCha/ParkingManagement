...
class SpaceNumberParser:
    ...
    def parse_range(start: str, end: str) -> tuple[list[str], bool]:
        ...
        space_numbers = [
            f"{start_prefix}{str(num).zfill(num_width)}"
    ...
    @staticmethod
    def parse_from_excel(file_content: bytes, sheet_name: str | None = None) -> list[dict[str, str]]:
        """
        从Excel文件中解析车位号
...
class SpaceCreationService:
    ...
    def create_spaces_from_range(
        ...
        # 检查是否已存在
        existing = set(ParkingSpace.objects.filter(
            parking_lot=parking_lot,
            space_number__in=space_numbers
        ...
        return len(created_numbers), len(skipped_numbers), created_numbers, skipped_numbers, True, '创建完成'
            ...
            match file_type:
                case 'txt' | 'md':
                    ...
                    skipped_numbers.extend(
                        space_number for space_number in space_numbers
                        if space_number in existing
                    )
                    ...
                    return len(created_numbers), len(skipped_numbers), created_numbers, skipped_numbers, failed_lines, True, '创建完成'
                ...
                case 'xlsx':
                    ...
                    # 收集跳过的车位号
                    skipped_numbers.extend(
                        info['space_number'] for info in spaces_info
    ...
    def create_spaces_from_file_simple(
        ...
        从文件创建车位（简化版本，保持向后兼容）
        """
        created, skipped, created_list, skipped_list, failed_lines, success, message = SpaceCreationService.create_spaces_from_file(
            parking_lot, file_content, file_type
        )
        if not success:
