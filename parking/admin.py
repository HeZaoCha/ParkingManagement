...
from parking.license_plate_models import (
    LicensePlateLocation, Province, City, VehicleAlertLog, WantedVehicle
)
...
class ParkingLotAdmin(admin.ModelAdmin):
    """停车场管理"""
    list_display = ['name', 'address', 'total_spaces', 'available_spaces_display', 'hourly_rate', 'is_active', 'created_at']
    list_filter = ['is_active', 'created_at']
    search_fields = ['name', 'address']
    readonly_fields = ['created_at', 'updated_at', 'available_spaces_display', 'occupied_spaces_display']
...
class ParkingSpaceAdmin(admin.ModelAdmin):
    """停车位管理"""
    list_display = ['space_number', 'parking_lot', 'space_type', 'is_occupied', 'is_reserved', 'created_at']
    ...
    fieldsets = (
        ('基本信息', {
            'fields': ('parking_lot', 'space_number', 'space_type')
        }),
        ('状态', {
...
class ParkingRecordAdmin(admin.ModelAdmin):
    """停车记录管理"""
    list_display = ['vehicle', 'parking_space', 'entry_time', 'exit_time', 'duration_display', 'fee', 'is_paid', 'operator']
    ...
    fieldsets = (
        ('基本信息', {
            'fields': ('vehicle', 'parking_space', 'operator')
        }),
        ('时间信息', {
            'fields': ('entry_time', 'exit_time', 'duration_display')
        }),
        ('费用信息', {
            'fields': ('fee', 'is_paid')
        }),
...
class PricingRuleAdmin(admin.ModelAdmin):
    """费率规则管理"""
    list_display = ['template', 'start_minutes', 'end_minutes', 'rate_per_hour', 'order']
...
class WantedVehicleAdmin(admin.ModelAdmin):
    """通缉车辆管理"""
    list_display = ['license_plate', 'vehicle_type', 'status', 'priority', 'created_at', 'created_by']
...
class VehicleAlertLogAdmin(admin.ModelAdmin):
    """车辆警报日志管理"""
    list_display = ['wanted_vehicle', 'parking_record', 'alert_time', 'is_handled', 'handled_by', 'handled_at']
