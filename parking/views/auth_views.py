...
def register_view(request):
    ...
    try:
        ...
        if not form.is_valid():
            return JsonResponse(
                {"success": False, "message": form.errors.as_text(), "errors": form.errors},
                status=400,
            )
        ...
        target = (
            ...
            if code_type == "email"
            else form.cleaned_data.get("phone")
        )
        code = form.cleaned_data.get("verification_code")

        verification = (
            VerificationCode.objects.filter(
                target=target, code_type=code_type, purpose="register", code=code, is_used=False
            )
            .order_by("-created_at")
            .first()
        )
        ...
        return JsonResponse(
            {"success": True, "message": "注册成功", "redirect_url": "/parking/customer/"}
        )
...
def send_verification_code(request):
    ...
    try:
        ...
        if recent_code:
            return JsonResponse(
                {"success": False, "message": "发送过于频繁，请稍后再试"}, status=429
            )

        # 创建验证码
        verification = VerificationCode.create_code(
            code_type=code_type, purpose=purpose, target=target
        )
        ...
        if code_type == "email":
            try:
                ...
                success = EmailService.send_verification_code(
                    email=target, code=verification.code, purpose=purpose_display
                )
                if not success:
                    verification.delete()
                    return JsonResponse(
                        {"success": False, "message": "邮件发送失败，请检查邮箱地址或联系管理员"},
                        status=500,
                    )
            except Exception as e:
                ...
                verification.delete()
                return JsonResponse(
                    {"success": False, "message": f"邮件发送失败: {str(e)}"}, status=500
                )
        ...
        # 获取验证码类型显示名称
        type_display = dict(VerificationCode.TYPE_CHOICES).get(
            code_type, "邮箱" if code_type == "email" else "手机"
        )
        ...
        return JsonResponse({"success": False, "message": f"发送失败: {str(e)}"}, status=500)
...
def verify_code(request):
    ...
    try:
        ...
        verification = (
            VerificationCode.objects.filter(
                target=target, code_type=code_type, purpose=purpose, code=code, is_used=False
            )
            .order_by("-created_at")
            .first()
        )
...
def forgot_password_view(request):
    ...
    try:
        ...
        if not user:
            ...
            return JsonResponse(
                {"success": True, "message": "如果该账户存在，验证码已发送到您的注册邮箱"}
            )

        # 检查用户是否有邮箱
        if not user.email:
            return JsonResponse(
                {"success": False, "message": "该账户未绑定邮箱，无法重置密码，请联系管理员"},
                status=400,
            )
        ...
        # 创建验证码
        verification = VerificationCode.create_code(
            code_type="email", purpose="reset_password", target=user.email
        )

        # 发送验证码邮件
        try:
            success = EmailService.send_verification_code(
                email=user.email, code=verification.code, purpose="重置密码"
            )
            if not success:
                verification.delete()
                return JsonResponse(
        ...
        return JsonResponse({"success": False, "message": f"处理失败: {str(e)}"}, status=500)
...
def reset_password_view(request):
    ...
    try:
        ...
        if not email:
            return JsonResponse({"success": False, "message": "缺少邮箱参数"}, status=400)
        ...
        try:
            ...
            return JsonResponse(
                {"success": False, "message": "用户不存在或账户已被禁用"}, status=400
            )

        # 验证验证码
        verification = (
            VerificationCode.objects.filter(
                target=email, code_type="email", purpose="reset_password", code=code, is_used=False
            )
            .order_by("-created_at")
            .first()
        )
        ...
        if not verification.is_valid():
            return JsonResponse(
                {"success": False, "message": "验证码已过期，请重新获取"}, status=400
            )
        ...
        return JsonResponse({"success": False, "message": f"重置失败: {str(e)}"}, status=500)
