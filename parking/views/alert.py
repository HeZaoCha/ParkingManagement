...
def wanted_vehicle_detail(request: HttpRequest, pk: int) -> HttpResponse:
    ...
    # 获取相关警报日志
    alert_logs = (
        VehicleAlertLog.objects.filter(wanted_vehicle=vehicle)
        .select_related("parking_record", "parking_record__parking_space__parking_lot")
...
def wanted_vehicle_edit(request: HttpRequest, pk: int = None) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        if request.content_type == "application/json":
            return JsonResponse(
                {"success": True, "message": "保存成功", "data": {"id": vehicle.id}}
            )
...
def wanted_vehicle_cancel(request: HttpRequest, pk: int) -> JsonResponse:
    ...
    return JsonResponse({"success": True, "message": "已取消通缉"})
