...
def schedule_upload(request):
    ...
    if not file.name.endswith((".xlsx", ".xls")):
        return JsonResponse(
            {"success": False, "message": "只支持Excel文件（.xlsx, .xls）"}, status=400
        )
        ...
        for row_idx, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
            ...
            try:
                ...
                try:
                    ...
                    raise ValueError(
                        f"时间格式错误: {start_time_str} 或 {end_time_str}，应为 HH:MM"
                    )
...
def schedule_list(request):
    """排班表列表"""
    schedules = (
        StaffSchedule.objects.select_related("user", "parking_lot")
        .filter(is_active=True)
        .order_by("parking_lot", "weekday", "start_time")
    )
