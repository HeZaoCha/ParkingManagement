...
from parking.forms import VehicleEntryForm, VehicleExitForm, VehicleQueryForm
from parking.models import ParkingLot, ParkingRecord, ParkingSpace, Vehicle, validate_license_plate
...
def api_vehicle_query(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        return api_response(
            success=True,
            data=status,
            message='查询成功' if status['found'] else '未找到该车辆'
        )
    ...
    try:
        ...
        for record in result.records:
            records.append({
                ...
                'exit_time': record.exit_time.strftime('%Y-%m-%d %H:%M') if record.exit_time else None,
        ...
        for record in data['recent_records']:
            ...
            if 'entry_time' in formatted_record and formatted_record['entry_time']:
                try:
                    ...
                    # 处理ISO格式字符串
                    if isinstance(formatted_record['entry_time'], str) and 'T' in formatted_record['entry_time']:
                        dt = datetime.fromisoformat(formatted_record['entry_time'].replace('Z', '+00:00'))
            ...
            if 'exit_time' in formatted_record and formatted_record['exit_time']:
                try:
                    from datetime import datetime
                    if isinstance(formatted_record['exit_time'], str) and 'T' in formatted_record['exit_time']:
                        dt = datetime.fromisoformat(formatted_record['exit_time'].replace('Z', '+00:00'))
...
def api_parking_lot_detail(request: HttpRequest, lot_id: int) -> JsonResponse:
    ...
    try:
        ...
        lot = ParkingLot.objects.only(
            'id', 'name', 'address', 'lot_type', 'floors', 
            'areas', 'total_spaces', 'hourly_rate'
...
def api_available_spaces(request: HttpRequest) -> JsonResponse:
    ...
    try:
        ...
        spaces = ParkingSpace.objects.filter(
            ...
            is_occupied=False,
            is_reserved=False
        ).order_by('space_number').values('id', 'space_number', 'space_type')
