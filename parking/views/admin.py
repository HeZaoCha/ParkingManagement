...
from parking.services import DashboardService, ParkingRecordService
...
def parking_lot_detail(request: HttpRequest, pk: int) -> HttpResponse:
    ...
    recent_records = ParkingRecord.objects.filter(
        ...
        'id', 'entry_time', 'exit_time', 'fee', 'is_paid',
    ...
    spaces = lot.parking_spaces.select_related().only(
        'id', 'space_number', 'floor', 'area', 'space_type', 
...
def parking_lot_delete(request: HttpRequest, pk: int) -> JsonResponse:
    ...
    try:
        ...
        if ParkingSpace.objects.filter(parking_lot=lot).exists():
            return JsonResponse({
                'success': False,
                'message': '该停车场下存在车位，无法删除。请先删除所有车位。'
            })
...
def parking_space_list(request: HttpRequest) -> HttpResponse:
    ...
    if search:
        queryset = queryset.filter(
            Q(space_number__icontains=search) | 
...
def vehicle_list(request: HttpRequest) -> HttpResponse:
    ...
    queryset = Vehicle.objects.annotate(
        record_count=Count('parking_records')
    ).order_by('-created_at')
...
def parking_record_checkout(request: HttpRequest, pk: int) -> JsonResponse:
    ...
    try:
        with transaction.atomic():
            ...
            logger.info(
                "用户 %s 处理出场: 车牌 %s, 费用 %s",
                request.user.username, record.vehicle.license_plate, record.fee
            )
