...
def pricing_template_edit(request, template_id=None):
    ...
    if template:
        template = PricingTemplate.objects.prefetch_related(
            "rules", "month_year_rates", "overtime_rates"
        ).get(id=template.id)
...
def _normalize_vehicle_type(vehicle_type: str) -> str:
    ...
    return mapping.get(
        vehicle_type,
        vehicle_type if vehicle_type in ["all", "standard", "disabled", "vip", "large"] else "all",
    )
...
def _normalize_rate_type(rate_type: str) -> str:
    ...
    return mapping.get(
        rate_type, rate_type if rate_type in ["month", "quarter", "year"] else rate_type
    )
...
def pricing_template_download(request):
    ...
    basic_header_fill = PatternFill(
        start_color="4472C4", end_color="4472C4", fill_type="solid"
    )  # 蓝色
    rule_header_fill = PatternFill(
        start_color="70AD47", end_color="70AD47", fill_type="solid"
    )  # 绿色
    month_header_fill = PatternFill(
        start_color="FFC000", end_color="FFC000", fill_type="solid"
    )  # 橙色
    overtime_header_fill = PatternFill(
...
def _generate_pdf_manual() -> bytes:
    ...
    try:
        ...
        story.append(
            Paragraph(
                "• <b>模板名称</b>：必填，唯一标识，如'标准阶梯收费'、'优惠收费'等", normal_style
            )
        )
        story.append(Paragraph("• <b>模板描述</b>：选填，模板的详细说明", normal_style))
        story.append(
            Paragraph("• <b>免费时长</b>：必填，停车多长时间内免费（单位：分钟）", normal_style)
        )
        story.append(
            Paragraph("• <b>每日收费上限</b>：选填，留空表示不设上限（单位：元）", normal_style)
        )
        ...
        story.append(
            Paragraph("• <b>规则起始分钟</b>：必填，规则开始生效的分钟数（包含）", normal_style)
        )
        story.append(Paragraph("• <b>规则结束分钟</b>：选填，留空表示无上限", normal_style))
        story.append(
            Paragraph(
                "• <b>每小时费率</b>：必填，该时间段的收费标准（单位：元/小时）", normal_style
            )
        )
        ...
        story.append(
            Paragraph(
                "  - 中文：全部、标准、残疾人、VIP、大型（系统会自动转换为英文）", normal_style
            )
        )
        ...
        story.append(
            Paragraph(
                "• <b>超时费用</b>：必填，超过每日上限后的收费标准（单位：元/小时）", normal_style
            )
        )
        story.append(
            Paragraph(
                "• <b>超时起始小时</b>：必填，超过多少小时后开始收取超时费用（默认24小时）",
                normal_style,
            )
        )
        story.append(Paragraph("• <b>车位类型</b>：必填，同费率规则中的车位类型", normal_style))
    ...
    for file_idx, file in enumerate(files, start=1):
        ...
        try:
            ...
            for row_idx, row in enumerate(
                ...
                if (
                    first_col
                    and first_col not in ["month", "quarter", "year"]
                    and not first_col.replace(".", "").isdigit()
                    and first_col not in ["", "None"]
                ):
                    # 模板基本信息行
                    ...
                    try:
                        ...
                        if len(row) > 2 and row[2]:
                            try:
                                ...
                                if free_minutes < 0 or free_minutes > 1440:
                                    errors.append(
                                        f"文件 '{file_name}' 第{row_idx}行：免费时长必须在0-1440分钟之间"
                                    )
                                    continue
                        ...
                        errors.append(
                            f"文件 '{file_name}' 第{row_idx}行模板基本信息解析错误: {str(e)}"
                        )
                        continue
                    ...
                    if len(row) >= 3 and row[0] is not None:
                        try:
                            ...
                            continue
                elif first_col in ["month", "quarter", "year"] or first_col in [
                    "月卡",
                    "季卡",
                    "年卡",
                ]:
            ...
            if template_data:
                ...
                if not _validate_template_name(template_name):
                    ...
                    error_count += 1
                    errors.append(
                        f"文件 '{file_name}': 模板 '{template_name}' 必须包含至少一条费率规则"
                    )
                else:
                    try:
