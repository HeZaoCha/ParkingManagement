...
from parking.user_models import VerificationCode
...
class VehicleEntryForm(forms.Form):
    ...
    def clean_license_plate(self) -> str:
        ...
        ).select_related('parking_space__parking_lot').first()
        
        if active_record:
            lot_name = active_record.parking_space.parking_lot.name
            raise ValidationError(
                f'该车辆已在 {lot_name} 停车，请先办理出场',
...
class ParkingLotForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'total_spaces': forms.NumberInput(attrs={
                'class': 'form-input',
                'min': '0',
            }),
...
class RegisterForm(forms.Form):
    ...
    def clean(self) -> dict:
        ...
        if code_type == 'email':
            if not email:
                raise ValidationError({
                    'email': '使用邮箱验证时，邮箱不能为空'
                })
        elif code_type == 'phone':
            if not phone:
                raise ValidationError({
                    'phone': '使用手机验证时，手机号不能为空'
                })
        ...
        if password:
            ...
            if password.lower() in common_passwords:
                raise ValidationError({
                    'password': '密码过于简单，请选择更复杂的密码'
                })
            
            # 检查重复字符（如：aaaaaaa, 11111111）
            if len(set(password)) < 3:
                raise ValidationError({
                    'password': '密码包含过多重复字符，请使用更复杂的密码'
            })
