...
class ReportService:
    ...
    def get_daily_stats(date: Optional[datetime] = None) -> dict[str, Any]:
        ...
        records = ParkingRecord.objects.filter(entry_time__gte=date_start, entry_time__lte=date_end)
    ...
    def get_range_stats(start_date: datetime, end_date: datetime) -> dict[str, Any]:
        ...
        Returns:
            dict: 统计数据字典
        """
        records = ParkingRecord.objects.filter(entry_time__gte=start_date, entry_time__lte=end_date)
        ...
        # 按停车场统计
        parking_lot_stats = (
            records.values("parking_space__parking_lot__name")
            .annotate(count=Count("id"), revenue=Sum("fee", filter=Q(is_paid=True)))
            .order_by("-count")
        )
    ...
    def get_parking_lot_stats(parking_lot_id: Optional[int] = None) -> dict[str, Any]:
        ...
        for lot in parking_lots:
            ...
            today_records = ParkingRecord.objects.filter(
                parking_space__parking_lot=lot, entry_time__gte=today_start
            )
            today_count = today_records.count()
            today_revenue = today_records.filter(is_paid=True).aggregate(total=Sum("fee"))[
                "total"
            ] or Decimal("0.00")
            ...
            result.append(
                {
                    ...
                    "occupancy_rate": round(occupied / lot.total_spaces * 100, 2)
    ...
    def get_chart_data(days: int = 7) -> dict[str, Any]:
        ...
        while current_date <= end_date_only:
            ...
            records = ParkingRecord.objects.filter(
                entry_time__gte=date_start, entry_time__lte=date_end
            )

            count = records.count()
            revenue = records.filter(is_paid=True).aggregate(total=Sum("fee"))["total"] or Decimal(
                "0.00"
            )
