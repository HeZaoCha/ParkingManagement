# 项目文档索引

**最后更新**: 2025-12-14  
**版本**: 2.0.0

欢迎来到停车场管理系统文档中心！本文档提供了项目所有文档的索引和导航。

---

## 📚 文档分类

### 🏗️ [架构文档](./architecture/)
系统架构、数据库设计、模块说明等核心架构文档。

- [系统架构](./architecture/ARCHITECTURE.md) - 整体架构设计（包含静态文件组织说明）
- [数据库设计](./architecture/DATABASE.md) - 数据库模型和关系
- [模块说明](./architecture/MODULES.md) - 各功能模块说明

### 🔧 [开发文档](./development/)
开发环境搭建、开发指南等开发相关文档。

- [开发指南](./development/DEVELOPMENT.md) - 开发环境搭建和开发流程

### 🧪 [测试文档](./testing/)
测试报告、测试配置、测试指南等测试相关文档。

- [测试报告](./testing/TEST_REPORT.md) - 完整的测试报告（237+个测试用例，100%通过，覆盖率73%+）
- [Bug修复报告](./testing/BUG_FIX_REPORT.md) - Bug修复详情
- [最终覆盖率报告](./testing/FINAL_COVERAGE_REPORT.md) - 代码覆盖率详情（从61%提升到75%）

### 🚀 [部署文档](./deployment/)
生产环境部署、服务器配置、运维相关文档。

- [Docker Compose 多环境配置指南](./deployment/DOCKER_COMPOSE_GUIDE.md) - 多环境配置最佳实践（基础配置+覆盖）
- [Docker 开发测试环境](./deployment/DEVELOPMENT_DOCKER.md) - Docker 开发测试环境完整指南（热重载、调试、测试）
- [快速部署指南](./deployment/QUICK_START.md) - Docker 生产环境快速部署
- [部署指南](./deployment/DEPLOYMENT.md) - 传统部署方式（非 Docker）
- [生产环境技术栈方案](./deployment/PRODUCTION_TECH_STACK.md) - 完整的技术栈方案（支持10万用户，包含最佳实践和优化建议）

### 📖 [用户指南](./user-guide/)
用户手册、操作指南、界面设计等用户相关文档。

- [用户手册](./user-guide/USER_GUIDE.md) - 用户操作指南
- [界面设计系统](./user-guide/UI_DESIGN_SYSTEM.md) - UI设计规范

### 🤝 [贡献指南](./contributing/)
开源贡献者指南、代码规范、提交流程等。

- [贡献指南](./contributing/CONTRIBUTING.md) - 如何参与项目贡献

### 🐛 [Bug修复记录](./bugfixes/)
项目开发过程中发现和修复的Bug记录。

- [Bug修复总结](./bugfixes/BUGFIX_SUMMARY.md) - Bug修复汇总

### 🔄 [重构记录](./refactoring/)
项目重构历史、重构总结等。

- [重构总结](./refactoring/RESTRUCTURE_SUMMARY.md) - 重构工作总结

### ⚡ [优化记录](./optimization/)
性能优化、前端优化、用户体验优化等优化记录。

- [优化最终报告](./optimization/FINAL_OPTIMIZATION_REPORT.md) - 所有优化工作的完整总结

### ✨ [功能文档](./features/)
新功能开发、功能优化、功能设计等文档。

- 功能详情请参考 [项目文档](./PROJECT_DOCUMENTATION.md)

### 📡 [API文档](./api/)
API接口文档、API使用指南等。

- [RESTful API文档](./api/API.md) - 完整的API接口文档（568行，包含所有接口说明）

---

## 🎯 快速导航

### 按角色查找文档

#### 👨‍💻 开发工程师
- [开发指南](./development/DEVELOPMENT.md) - 开始开发
- [系统架构](./architecture/ARCHITECTURE.md) - 了解架构
- [API文档](./api/API.md) - API使用
- [代码规范](./contributing/CONTRIBUTING.md) - 编码规范
- [测试报告](./testing/TEST_REPORT.md) - 测试覆盖和测试结果

#### 👔 项目经理
- [项目文档](./PROJECT_DOCUMENTATION.md) - 项目概览
- [优化报告](./optimization/FINAL_OPTIMIZATION_REPORT.md) - 优化完成报告
- [变更日志](../CHANGELOG.md) - 版本更新
- [安全文档](./SECURITY.md) - 安全策略

#### 👤 项目用户
- [用户手册](./user-guide/USER_GUIDE.md) - 使用指南
- [界面设计](./user-guide/UI_DESIGN_SYSTEM.md) - 界面说明

#### 🌟 开源贡献者
- [贡献指南](./contributing/CONTRIBUTING.md) - 如何贡献
- [开发指南](./development/DEVELOPMENT.md) - 开发环境
- [代码规范](./contributing/CONTRIBUTING.md) - 编码规范

---

## 📋 核心文档

### 必读文档
1. [README.md](../README.md) - 项目介绍
2. [开发指南](./development/DEVELOPMENT.md) - 开发环境搭建
3. [系统架构](./architecture/ARCHITECTURE.md) - 架构设计
4. [用户手册](./user-guide/USER_GUIDE.md) - 用户指南

### 重要文档
- [部署指南](./deployment/DEPLOYMENT.md) - 生产部署
- [API文档](./api/API.md) - API接口
- [安全文档](./SECURITY.md) - 安全策略
- [变更日志](../CHANGELOG.md) - 版本历史

---

## 🔍 文档搜索

### 按主题搜索

**性能优化**
- [优化最终报告](./optimization/FINAL_OPTIMIZATION_REPORT.md)

**Bug修复**
- [Bug修复总结](./bugfixes/BUGFIX_SUMMARY.md)

**重构记录**
- [重构总结](./refactoring/RESTRUCTURE_SUMMARY.md)

---

## 📝 文档维护

### 文档更新原则
1. **及时更新**: 功能变更后及时更新相关文档
2. **保持同步**: 确保文档与代码同步
3. **分类清晰**: 按分类组织文档
4. **易于查找**: 提供清晰的索引和导航

### 文档贡献
欢迎贡献文档改进！请参考 [贡献指南](./contributing/CONTRIBUTING.md)。

---

## 📞 获取帮助

- **开发问题**: 查看 [开发指南](./development/DEVELOPMENT.md)
- **使用问题**: 查看 [用户手册](./user-guide/USER_GUIDE.md)
- **API问题**: 查看 [API文档](./api/API.md)
- **贡献问题**: 查看 [贡献指南](./contributing/CONTRIBUTING.md)

---

**文档维护**: HeZaoCha  
**最后更新**: 2025-12-15
